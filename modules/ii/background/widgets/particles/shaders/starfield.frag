    ...
    for (int layer = 1; layer <= 3; layer++) {
        ...
        for (int y = -1; y <= 1; y++) {
            for (int x = -1; x <= 1; x++) {
                ...
                float maxDist = starSize * (3.5 + particleBlur * 3.0);
                if (dist < maxDist) {
                    float core = smoothstep(starSize, 0.0, dist);
                    float glow = exp(-dist / (starSize * (1.2 + particleBlur * 1.5))) * 0.5;

                    float spike = 0.0;
                    if (rnd.x > 0.65) {
                        float crossSpike = max(0.0, 1.0 - abs(p.x) / (starSize * 3.5)) * max(0.0, 1.0 - abs(p.y) / (starSize * 0.8))
                                         + max(0.0, 1.0 - abs(p.y) / (starSize * 3.5)) * max(0.0, 1.0 - abs(p.x) / (starSize * 0.8));
